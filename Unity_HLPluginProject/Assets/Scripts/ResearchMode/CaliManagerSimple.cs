...
public class CaliManagerSimple : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Matrix _measurementNoiseCovMat_tool = Matrix.IdentityMatrix(6, 6, 1e-3);
        Matrix _processNoiseCovMat_tool = Matrix.IdentityMatrix(6, 6, 5e-2);
        ...
        localstart_tool = new Vector4(0, 0, 0.125f, 1);
        localend_tool = new Vector4(0, 0, 0, 1);
    }
    ...
    public void UpdateTrackedTool_simple(double[] matTransforms, Matrix4x4 cameraToWorld)
    {
        ...
        Matrix4x4 tool_W_RH = Matrix4x4.identity;
        ...
        for (int i = 0; i < numberOfTools; i++)
        {
            ...
            Matrix4x4 temp = MatrixUtilities.FillMatrixWithDoubles(matrixElements,
                MatrixUtilities.MatrixEntryOrder.RowMajor, MatrixUtilities.MatrixUnits.m);
            if (temp != Matrix4x4.identity)
            {
                // convert to matrix 
                tool_W_RH = temp;
            ...
            // else tool_W_RH is identity
        }

        // for relative pose calibration
        if (TCPCommunication.message != "" && TCPCommunication.message.Split(',').Length == 13)
        {
            ...
            if (tool_W_RH != Matrix4x4.identity && tool_W.m23 > 0 && toolID==0)
            {
        ...
        // for tool geomatry initialisation
        else if (TCPCommunication.message != "" && TCPCommunication.message.Split(',').Length == 8)
        {
            ...
            GameObject tipsphere = caliSpheres.transform.Find("tip").gameObject;
            tipsphere.transform.localPosition = localstart_tool;
        ...
        // for purely tool tracking
        else if (!_forceToHide && tracking)
        {
            if (tool_W_RH != Matrix4x4.identity)
            {
                ...
                toolText.text = toolID.ToString();
            }
            else
            {
                _kalmanFiltertool.Predict();
                Vector3 toolstart = new Vector3((float)_kalmanFiltertool.X[0, 0], (float)_kalmanFiltertool.X[1, 0], (float)_kalmanFiltertool.X[2, 0]);
                Vector3 toolend = new Vector3((float)_kalmanFiltertool.X[3, 0], (float)_kalmanFiltertool.X[4, 0], (float)_kalmanFiltertool.X[5, 0]);
